...
- **Tailwind standalone binaries exist** for the platforms that matter (v4.3.3: `tailwindcss-linux-x64`, `-linux-arm64`, `-linux-x64-musl`, `-macos-arm64`), confirmed against the GitHub releases API. v4 is CSS-first, so no JS config file lands in the Python repo.
